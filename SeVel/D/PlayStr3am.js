...
settings.createBool('disableSampleList', 'Ocultar Listas Premium', false, function(v) {
  service.disableSampleList = v;
});
settings.createBool('disableSampleXMLList', 'Ocultar Listas Gratuitas', false, function(v) {
  service.disableSampleXMLList = v;
});
settings.createBool('disableProvidersList', 'Mostrar Lista de Provedores', false, function(v) {
  service.disableProviderList = v;
});
settings.createBool('disableEPG', 'No Buscar EPG', true, function(v) {
  service.disableEPG = v;
});
settings.createString('acestreamIp', 'NO TOCAR', '192.168.0.93', function(v) {
  service.acestreamIp = v;
});
settings.createBool('debug', 'Registro de Depuracion', true, function(v) {
  service.debug = v;
});
settings.createBool('disableMyFavorites', 'Mostrar Tus Favoritos', false, function(v) {
  service.disableMyFavorites = v;
});
settings.createAction('cleanFavorites', 'Eliminar Todos Tus Favoritos', function() {
  store.list = '[]';
...
function addOptionForRemovingFromMyFavorites(page, item, title, pos) {
  item.addOptAction('Eliminar \'' + title + '\' de Tus Favoritos', function() {
    ...
    popup.notify('\'' + title + '\' a sido Eliminado de Tus Favoritos.', 2);
    list.splice(pos, 1);
    store.list = JSON.stringify(list);
    page.redirect(plugin.id + ':Favoritos');
  });
}
...
function fill_fav(page) {
  ...
  if (!list || !list.toString()) {
    page.error('Aun No Tienes Favoritos');
    return;
  }
  var pos = 0;
  for (var i in list) {
...
function showPlaylist(page) {
  ...
  if (!playlist || !playlist.toString()) {
    popup.notify('Puedes Agregar tu Propia Lista M3U desde el Menu Lateral', 5);
  }
...
function readAndParseM3U(page, pl, m3u) {
  ...
  page.loading = true;
  if (!m3u) {
    page.metadata.title = 'Buscando Lista M3U...';
  ...
  for (var i = 0; i < m3u.length; i++) {
    page.metadata.title = 'Cargando Lineas ' + i + ' of ' + m3u.length;
    line = m3u[i].trim();
...
function addActionToTheItem(page, menuText, id, type) {
  page.options.createAction('addPlaylist' + type, menuText, function() {
    var result = popup.textDialog('Ingresa la URL de tu Lista:\n' +
    ...
    if (!result.rejected && result.input) {
      ...
      var result = popup.textDialog('Ingresa el Nombre de tu Lista:', true, true);
      if (!result.rejected && result.input) {
        var entry = JSON.stringify({
          title: encodeURIComponent(result.input),
          link: type.toLowerCase() + ':' + encodeURIComponent(link),
          icon: 'https://i.ibb.co/N6wdstw/favoritos.jpg',
        });
        playlists.list = JSON.stringify([entry].concat(eval(playlists.list)));
        popup.notify('La Lista \'' + result.input + '\' ha sido Agregada.', 2);
        page.flush();
        page.redirect(plugin.id + ':start');
      }
    }
...
new page.Route(plugin.id + ':loadOnePlaylist', function(page) {
  ...
  page.loading = true;
  page.metadata.title = 'Descargando Lista...';
...
new page.Route(plugin.id + ':start', function(page) {
  ...
  if (!service.disableSampleList) {
			page.model.contents = 'grid';
    var item = page.appendItem('m3u:https://bit.ly/4b2Or2r:IPTV Premium', 'directory', {
      title: 'IPTV Premium',
      icon: 'https://i.ibb.co/nc8LffK/IPTVP.jpg',
    });
  }

  if (!service.disableSampleXMLList) {
			page.model.contents = 'grid';
    var item = page.appendItem('m3u:https://bit.ly/3NhUWoX:IPTV Gratis', 'directory', {
      title: 'IPTV Gratis',
      icon: 'https://i.ibb.co/hLnRZCx/IPTVG.jpg',
    });
  }

  if (!service.disableSampleList) {
			page.model.contents = 'grid';
    var item = page.appendItem('m3u:https://bit.ly/4cojpDx:Peliculas', 'directory', {
      title: 'Peliculas',
      icon: 'https://i.ibb.co/PwM78RH/Peliculas.jpg',
    });
  }

  if (!service.disableSampleXMLList) {
			page.model.contents = 'grid';
    var item = page.appendItem('m3u:https://bit.ly/3VolQPt:Tu Zona Gratis', 'directory', {
      title: 'Tu Zona Gratis',
      icon: 'https://i.ibb.co/0hn8rVp/TuZonaG.jpg',
    });
  }

  if (!service.disableSampleList) {
			page.model.contents = 'grid';
    var item = page.appendItem('m3u:https://archive.org/download/nombres_series/nombres_series.m3u:Tu Zona Premium', 'directory', {
      title: 'Tu Zona Premium',
      icon: 'https://i.ibb.co/M7bGD6N/TuZona.jpg',
    });
  }

  if (service.disableMyFavorites) {
  page.appendItem('', 'separator', {
    title: '==★== Tu Entretenimiento Sin Límites ==★==',
  });
  ...
  addActionToTheItem(page, 'Agrega una Lista M3U', 'ElSuacht', 'M3U');

  // menu to delete playlists
  page.options.createAction('rmPlaylist', 'Eliminar Listas...', function() {
    var list = eval(playlists.list);
    for (var i in list) {
      var result = popup.message('Quieres Eliminar \'' + decodeURIComponent(JSON.parse(list[i]).title) + '\' ?', true, true);
      if (result) {
        popup.notify('\'' + decodeURIComponent(JSON.parse(list[i]).title) + '\' Ha sido Eliminada.', 2);
        list.splice(i, 1);
    ...
    if (!i) popup.notify('No hay Listas para Eliminar', 2);
  });
